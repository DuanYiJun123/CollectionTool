package com.cloudwalk.tool.idcardAccu;
...
public class IDCardFrontToolAccu {
	private static final String table = "testcollectionidcardfrontaccu";
	private DBHelper dbHelper;
	private String root;

	public IDCardFrontToolAccu(String url, String driver, String user, String password, String root) {
	...
	public void uploadAll() {
		...
		if (chidren != null) {
			...
			Arrays.asList(chidren).stream().forEach(resource -> {
				...
				addBankOcr(resource, conn);
				updateBankOcrSrcBase64(resource, conn);
			});
			if (conn != null) {
				try {
					conn.close();
			...
			System.out.println("NO pic! check the pic path!");
		}
	}
	...
	private void createTableIfNotExists(Connection conn0) {
		...
		try {
			...
			builder.append("create table testcollectionidcardfrontaccu (");
	...
	public void deleteBankOcr(Connection conn) {
		try {
	...
	public void updateBankOcrSrcBase64(String resource, Connection conn) {
	...
	public void addBankOcr(String resource, Connection conn) {
		File file = new File(root + File.separator + resource);
		String name = file.getName();
