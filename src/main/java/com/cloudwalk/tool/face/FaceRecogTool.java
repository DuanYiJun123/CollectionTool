package com.cloudwalk.tool.face;
...
public class FaceRecogTool {
	private static final String table = "testcollectionfacerecog";
	private DBHelper dbHelper;
	String root;

	public FaceRecogTool(String url, String driver, String user, String password, String root) {
	...
	public void uploadAll() {
		...
		if (chidren != null) {
			...
			Arrays.asList(chidren).stream().forEach(resource -> {
				...
				addFaceRecog(resource, conn);
				updateFaceRecogSrcBase64(resource, conn);
			});
			if (conn != null) {
				try {
					conn.close();
	...
	private void createTableIfNotExists(Connection conn0) {
		...
		try {
			...
			builder.append("create table testcollectionfacerecog (");
	...
	private void deleteFaceRecog(Connection conn) {
		try {
	...
	public void updateFaceRecogSrcBase64(String resource, Connection conn) {
	...
	public void addFaceRecog(String resource, Connection conn) {
		File file = new File(root + File.separator + resource);
		String name = file.getName();
