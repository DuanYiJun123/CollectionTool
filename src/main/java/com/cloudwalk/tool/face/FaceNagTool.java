...
public class FaceNagTool {
	private static final String table = "testcollectionfaceneg1000";
	private DBHelper dbHelper;
	String root;

	public FaceNagTool(String url, String driver, String user, String password, String root) {
	...
	public void uploadAll() {
		...
		if (chidren != null) {
			...
			Arrays.asList(chidren).stream().forEach(resource -> {
				...
				addFaceBase(resource, conn);
				updateFaceBaseSrcBase64(resource, conn);
	...
	private void createTableIfNotExists(Connection conn0) {
		...
		try {
			...
			builder.append("create table testcollectionfaceneg1000 (");
	...
	private void deleteFaceBase(Connection conn) {
		try {
	...
	public void updateFaceBaseSrcBase64(String resource, Connection conn) {
	...
	public void addFaceBase(String resource, Connection conn) {
		File file = new File(root + File.separator + resource);
		String name = file.getName();
