...
public class IDCardFrontTool {
	private static final String table = "testcollectionidcardfront";
	private DBHelper dbHelper;
	private String root;

	public IDCardFrontTool(String url, String driver, String user, String password, String root) {
	...
	private void createTableIfNotExists(Connection conn0) {
		...
		try {
			...
			builder.append("create table testcollectionidcardfront (");
	...
	public void updateBankOcrSrcBase64(String resource, Connection conn) {
		...
		try (PreparedStatement prepareStatement = conn
			...
			if (rs.next()) {
				...
				CLOB clob = (CLOB) rs.getClob("base64");
